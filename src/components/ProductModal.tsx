...
const ProductModal: React.FC<Props> = ({ product, isOpen, onClose }) => {
  ...
  const MODAL_CSS = `
    ...
    @media (min-width: 641px) {
      .pm-backdrop { align-items: center !important; padding: 1rem !important; }
      .pm-shell    { border-radius: 18px !important; max-height: 92vh !important; }
    }
    @media (max-width: 640px) {
      .pm-inner { grid-template-columns: 1fr !important; overflow-y: auto; }
      .pm-inner > div:first-child { border-right: none !important; border-bottom: 1px solid #f0ece4; }
      .pm-inner > div:first-child > div:first-child { min-height: 230px !important; max-height: 52vw !important; }
    }
  `;
  ...
  return (
    <>
      {/* Backdrop */}
      <div onClick={e => e.target === e.currentTarget && onClose()} className="pm-backdrop"
        ...
        <div className="pm-shell"
          style={{ position: 'relative', width: '100%', maxWidth: '960px', borderRadius: '18px 18px 0 0', overflow: 'hidden', background: 'white', boxShadow: '0 -20px 60px rgba(0,0,0,0.3)', transform: visible ? 'translateY(0)' : 'translateY(40px)', transition: 'transform 0.45s cubic-bezier(0.16,1,0.3,1)', maxHeight: '95dvh', display: 'flex', flexDirection: 'column' }}>
          ...
          <div className="pm-inner"
            ...
            <div style={{ background: '#fff', display: 'flex', flexDirection: 'column', borderRight: '1px solid #f0ece4' }}>
              ...
              <div onClick={() => { setLbIdx(imgIdx); setLightbox(true); }}
                style={{ flex: 1, minHeight: '280px', position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#fafaf9', cursor: 'zoom-in', overflow: 'hidden' }}>
                <img key={imgIdx} src={imgs[imgIdx]} alt={product.name}
                  style={{ maxWidth: '85%', maxHeight: '320px', objectFit: 'contain', animation: 'pmFadeIn 0.22s ease', userSelect: 'none' }} />
            ...
            {/* RIGHT — amber panel */}
            <div style={{ background: 'linear-gradient(160deg, #fffbeb 0%, #fef9f0 60%, #fef3c7 100%)', padding: '1.75rem 1.5rem', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '0.9rem', borderLeft: '1.5px solid #fde68a' }}>
              ...
              <div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.45rem' }}>
                  {specs.map(s => (
                    <div key={s.label} style={{ padding: '0.5rem 0.75rem', background: 'rgba(255,255,255,0.72)', borderRadius: '8px', border: '1px solid rgba(251,191,36,0.4)', borderLeft: '3px solid #d97706', backdropFilter: 'blur(4px)' }}>
              ...
              {product.tags?.length > 0 && (
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.3rem' }}>
                  {product.tags.map(tag => (
                    <span key={tag} style={{ padding: '0.18rem 0.55rem', borderRadius: '999px', background: 'rgba(255,255,255,0.7)', border: '1px solid rgba(217,119,6,0.3)', color: '#92400e', fontSize: '0.62rem', fontWeight: 600, fontFamily: 'system-ui, sans-serif' }}>#{tag}</span>
                  ))}
                </div>
              )}
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.65rem' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', border: '1.5px solid rgba(217,119,6,0.35)', borderRadius: '8px', overflow: 'hidden', background: 'rgba(255,255,255,0.7)' }}>
                  <button onClick={() => setQty(Math.max(1, qty - 1))} style={{ width: '2.4rem', height: '2.4rem', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'none', border: 'none', cursor: 'pointer', color: '#78716c' }}>
                  ...
                  <button onClick={() => setQty(qty + 1)} style={{ width: '2.4rem', height: '2.4rem', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'none', border: 'none', cursor: 'pointer', color: '#78716c' }}>
          ...
          <div style={{ position: 'absolute', bottom: '1.5rem', left: '50%', transform: 'translateX(-50%)', display: 'flex', gap: '8px' }}>
            {imgs.map((img, i) => (
              <div key={i} onClick={e => { e.stopPropagation(); setLbIdx(i); }}
                style={{ width: '3rem', height: '3rem', borderRadius: '7px', overflow: 'hidden', border: `2px solid ${i === lbIdx ? '#d97706' : 'rgba(255,255,255,0.2)'}`, cursor: 'pointer', opacity: i === lbIdx ? 1 : 0.55, transition: 'all 0.2s', flexShrink: 0 }}>
                <img src={img} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
              </div>
            ))}
          </div>
          <div style={{ position: 'absolute', top: '1rem', left: '50%', transform: 'translateX(-50%)', padding: '0.25rem 0.75rem', borderRadius: '999px', background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.15)', color: 'rgba(255,255,255,0.75)', fontSize: '0.7rem', fontWeight: 600, fontFamily: 'system-ui, sans-serif' }}>
            {lbIdx + 1} / {imgs.length}
          </div>
