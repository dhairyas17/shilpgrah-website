...
const CollectionModal: React.FC<Props> = ({ product, isOpen, onClose }) => {
  ...
  const MODAL_CSS = `
    ...
    @media (min-width: 641px) {
      .cm-backdrop { align-items: center !important; padding: 1rem !important; }
      .cm-shell    { border-radius: 18px !important; max-height: 92vh !important; }
    }
    @media (max-width: 640px) {
      .cm-inner { grid-template-columns: 1fr !important; overflow-y: auto; }
      .cm-inner > div:first-child { border-right: none !important; border-bottom: 1px solid #f0ece4; }
      .cm-inner > div:first-child > div:first-child { min-height: 230px !important; max-height: 52vw !important; }
    }
  `;

  return (
    <>
      <div onClick={e => e.target === e.currentTarget && onClose()} className="cm-backdrop"
            ...
            <div style={{ background: '#fff', display: 'flex', flexDirection: 'column', borderRight: '1px solid #f0ece4' }}>
              <div onClick={() => { setLbIdx(imgIdx); setLightbox(true); }}
                ...
                <img key={imgIdx} src={imgs[imgIdx]} alt={product.name}
                  style={{ maxWidth: '85%', maxHeight: '320px', objectFit: 'contain', animation: 'cmFadeIn 0.22s ease', userSelect: 'none' }} />
            ...
            <div style={{ background: 'linear-gradient(160deg, #fffbeb 0%, #fef9f0 60%, #fef3c7 100%)', padding: '1.75rem 1.5rem', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '0.9rem', borderLeft: '1.5px solid #fde68a' }}>
              ...
              <div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.45rem' }}>
                  {specs.map(s => (
                    <div key={s.label} style={{ padding: '0.5rem 0.75rem', background: 'rgba(255,255,255,0.72)', borderRadius: '8px', border: '1px solid rgba(251,191,36,0.4)', borderLeft: '3px solid #d97706' }}>
              ...
              <div>
                ...
                <p style={{ fontSize: '0.79rem', color: '#78716c', lineHeight: 1.8, fontFamily: 'system-ui, sans-serif' }}>
                  {expanded ? product.longDescription : `${product.longDescription.substring(0, 160)}...`}
                </p>
      ...
      {lightbox && (
        <div onClick={() => setLightbox(false)}
          ...
          <div style={{ position: 'absolute', bottom: '1.5rem', left: '50%', transform: 'translateX(-50%)', display: 'flex', gap: '7px' }}>
            {imgs.map((img, i) => (
          ...
          <div style={{ position: 'absolute', top: '1rem', left: '50%', transform: 'translateX(-50%)', padding: '0.25rem 0.75rem', borderRadius: '999px', background: 'rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.75)', fontSize: '0.7rem', fontWeight: 600, fontFamily: 'system-ui, sans-serif' }}>
            {lbIdx + 1} / {imgs.length}
          </div>
